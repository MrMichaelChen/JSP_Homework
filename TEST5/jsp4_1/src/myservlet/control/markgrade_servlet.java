...
/**
 * Servlet implementation class markgrade_servlet
 */
@WebServlet("/markgrade_servlet")
public class markgrade_servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public markgrade_servlet() {
        super();
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException{
	    ...
	    String teacher_name=request.getParameter("student");
	    ...
	    try {
			while (resultSet2.next()){
				String TeacherID1=resultSet2.getString("TeacherID").trim();
			    String course_id=resultSet2.getString("Coursenumber").trim();
			    if(TeacherID1.equals(user_id)){
					courseid=course_id;
				}
			}
		} catch (SQLException e1) {
		...
		try {
		       statement.executeQuery("insert into [student].[dbo].[choose_course] values ('"+teacherid+"','"+courseid+"', '"+grade+"')");
		} catch (SQLException e) {
		       e.printStackTrace();
		}
		response.sendRedirect("mark_success.jsp");
	  }
